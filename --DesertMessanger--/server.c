...
int main(int argc, char * argv[])
{
    ...
    puts("입력모드 입니다."); //사용자에게 알려주기
    puts("이용방법\n>help");
    flag = 1; // 채팅방식 컨트롤 변수  (Binary처럼 동작)
    ...
    while(-1)
    {
        if(flag == 1) // 입력모드 , 전송모드
            while(1)
            {
                ...
                printf("\n%s> ",servUserName);
                fgets(writeMessage, BUF_SIZE, stdin); writeMessage[strlen(writeMessage)-1] = '\0'; // 입력

                if(strcmp(writeMessage, "help") == 0) //help 이라면
                    _help();

                if(strcmp(writeMessage, "quit") == 0) //quit 이라면
                {
                    printf("종료합니다.\n");
                ...
                if(strcmp(writeMessage, "pass") == 0) // pass 이라면
                {
            ...
            write(clnt_sock, writeMessage,sizeof(writeMessage)); // clnt에게 입력한 데이터 전송
        ...
        if(flag == 0) // 관전모드, 수신모드
            while(1)
            {
                ...
                read(clnt_sock, readMessage,sizeof(readMessage)); // 수신하기
                ...
                if(strcmp(readMessage, "pass") == 0) // pass 이라면
                {
                    flag = 1;
                    puts("입력모드 입니다");
                    break;
